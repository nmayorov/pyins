...
from pyins.inertial_sensor import InertialSensorModel, InertialSensorError


def test_InertialSensor():
    s = InertialSensorModel()
    ...
    s = InertialSensorModel(bias_sd=0.1, bias_walk=0.2)
    ...
    s = InertialSensorModel(bias_sd=[0.1, 0.0, 0.2], bias_walk=[0.0, 0.0, 0.01],
                            noise=[0.0, 0.02, 0.0])
...
def test_ImuErrors():
    ...
    imu_errors = InertialSensorError(bias=[0.0, 0.2, 0.0])
    ...
    for sensor_type in ['rate', 'increment']:
        imu_errors = InertialSensorError()
        assert (imu_errors.apply(readings, sensor_type) == readings).all(None)

        imu_errors = InertialSensorError(bias_walk=0.1, rng=0)
        ...
        imu_errors = InertialSensorError(transform=np.diag([1.1, 1.0, 1.0]))
        readings_with_error = imu_errors.apply(readings, sensor_type)
        assert (readings_with_error[0] == 1.1 * readings[0]).all(None)
        assert (readings_with_error[[1, 2]] == readings[[1, 2]]).all(None)

        imu_errors = InertialSensorError(noise=[0.0, 0.0, 0.1])
        readings_with_error = imu_errors.apply(readings, sensor_type)
...
def test_ImuErrors_from_inertial_sensor_model():
    model = InertialSensorModel(bias_sd=[0.0, 1.0, 1.0],
